...
class AgentSession {
  ...
  render(now: number): void {
    ...
    if (lat == null) {
      ...
      if (lat > 1500) { level = 'bad'; this.gaugeVerdict.textContent = 'Frozen \u2717'; }
      else if (lat > 200) { level = 'bad'; this.gaugeVerdict.textContent = 'Lagging \u2717'; }
      else if (lat > 60) { level = 'slow'; this.gaugeVerdict.textContent = 'Lagging'; }
      else { level = 'good'; this.gaugeVerdict.textContent = 'Instant \u2713'; }
    }
...
function formatLatency(ms: number): { text: string; unit: string } {
  if (ms < 1000) return { text: ms.toFixed(0), unit: 'ms' };
  if (ms < 9500) return { text: (ms / 1000).toFixed(1), unit: 's' };
